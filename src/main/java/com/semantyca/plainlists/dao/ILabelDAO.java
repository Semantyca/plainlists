...
import com.semantyca.plainlists.api.ILabel;
...
public interface ILabelDAO extends IDAO {
    ...
    @SqlQuery("SELECT * FROM " + ILabel.TABLE_FULL_NAME + " as s WHERE s.identifier = :identifier;")
    @RegisterColumnMapper(LabelMapper.class)
    Label findByIdentifier(@Bind("identifier") String identifier);

    @SqlQuery("SELECT * FROM " + ILabel.TABLE_FULL_NAME + " as s  LIMIT :limit OFFSET :offset;")
    @RegisterColumnMapper(LabelMapper.class)
    List<Label> findAll(@Bind("limit") int limit, @Bind("offset") int offset);

    @SqlQuery("SELECT count(s.id) FROM " + ILabel.TABLE_FULL_NAME + " as s")
    long getCountOfAll();

    @SqlUpdate(
            "CREATE TABLE " + ILabel.TABLE_FULL_NAME +
            "(" +
    ...
    @SqlUpdate("INSERT INTO " + ILabel.TABLE_FULL_NAME + " (source, name, path, last_mod_date, last_mod_user, reg_date, title, author)" +
    ...
    @SqlUpdate("INSERT INTO " + ILabel.TABLE_FULL_NAME + "(color, category, hidden, loc_name, identifier, last_mod_date, last_mod_user, reg_date, title, author)VALUES (?,?,?,?::jsonb,?,?,?,?,?,?);")
    @GetGeneratedKeys("id")
